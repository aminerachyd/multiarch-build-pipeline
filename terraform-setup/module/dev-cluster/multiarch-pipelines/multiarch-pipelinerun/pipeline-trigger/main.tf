...
if [ $EXIT_CODE -eq 0 ]; then
  ...
  spec:
    ...
    - bindings:
      ...
      - params:
        - name: filter
          value: header.match('X-GitHub-Event', 'push') && body.ref == 'refs/heads/master' && body.repository.full_name == '${var.project-name}-dev/${var.app-name}'
        - name: overlays
          value: null
        ref:
